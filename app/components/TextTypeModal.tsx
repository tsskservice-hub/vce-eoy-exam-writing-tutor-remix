      ...
      <span>
        ...
        <Link
          ...
          onClick={(e) => {
            ...
            // 文法ページから戻ってきたときにどのモーダルを開いていたか復元できるようキー名を保存
...
export default function TextTypeModal({
  ...
  return (
    ...
    >
      <div
        className="bg-white p-6 w-full max-w-xl rounded-xl shadow-2xl relative max-h-[85vh] overflow-y-auto box-border"
      ...
      >
        ...
        <h3 className="mt-0 text-slate-800 text-xl font-bold pr-8">{title}</h3>

        {/* キーポイントセクション */}
        <div className="mt-4 bg-slate-50 p-4 rounded-lg border border-slate-200">
          <h4 className="m-0 mb-2 font-bold text-slate-800 text-sm sm:text-base">
            📌 Key points
          </h4>
          <p className="m-0 text-sm text-slate-700 mb-3 leading-relaxed">
            <strong>✨</strong> {purpose}
          </p>
          <ul className="m-0 pl-5 text-slate-700 space-y-2">
                  ...
                  <li key={index} className="mb-2">
                    ...
                    <ul className="mt-1 pl-5 list-disc space-y-1">
                      {mainTextSubItems.map((sub, subIndex) => (
                        <li key={subIndex} className="text-xs sm:text-sm text-slate-600">
                          {sub}
                        </li>
                      ))}
                    </ul>
        ...
        <div className="mt-4 bg-slate-50 p-4 rounded-lg border border-slate-200">
          ...
          {sampleImage ? (
                ...
                <>
                  <div className="text-center mb-3">
                    <a
                      ...
                      className="inline-block bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium no-underline transition-colors shadow-sm"
                    >
                      📄 View PDF Sample in New Tab ↗
                    </a>
                  </div>
                  <div className="border border-slate-300 rounded-lg overflow-hidden h-[400px] sm:h-[500px]">
                    <iframe
                      src={sampleImage}
                      width="100%"
                      height="100%"
            ...
            <p className="italic text-slate-600 bg-white p-3 rounded-md border-l-4 border-blue-600 m-0 whitespace-pre-wrap text-sm">
              {sample}
            </p>
          )}
        </div>
